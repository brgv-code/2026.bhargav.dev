...
import type { PayloadPostListItem, PayloadWorkExperience, PayloadFavorite } from "@/lib/payload";
...
export function HomePageLayout({
  ...
  const [activeTab, setActiveTab] = useState<"writing" | "projects" | "experience" | "favorites">("writing");
  const { playClick } = useSound();

  const switchTab = (tab: "writing" | "projects" | "experience" | "favorites") => {
    playClick();
    setActiveTab(tab);
  };
      ...
      <main className="lg:col-span-8 min-h-0 flex flex-col overflow-hidden">
        ...
        >
          <div className="flex flex-col space-y-6">
            {posts.length === 0 ? (
              <p className="text-[var(--editorial-text-muted)] text-[15px] py-8">No posts yet. Check back soon.</p>
            ) : (
              posts.map((post, i) => (
                <BlogCard
                  key={post.id}
                  ...
                  date={formatPostDate(post.publishedAt ?? post.updatedAt ?? post.createdAt)}
            ...
            <div className="flex flex-col sm:flex-row sm:gap-12">
              ...
              <div>
                ...
                <p className="text-sm font-light text-[var(--editorial-text-muted)] leading-relaxed max-w-2xl">
                  Architecting the primary UI and middle-tier API layer. Bridging
                  the gap between AI outputs and type-safe state.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-12">
            {favorites.length === 0 ? (
              <p className="text-sm text-[var(--editorial-text-muted)]">
                No favorites yet.{" "}
                <Link href="/favorites" className="text-[var(--editorial-accent)] hover:underline">
